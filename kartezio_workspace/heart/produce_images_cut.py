...
output_dir = "C:\\Module9\\Project\\module9-dsai\\kartezio_workspace\\heart\\experiment_dataset"
os.makedirs(output_dir, exist_ok=True)

# Function to save slices from a normal distribution around the middle slice
def save_normal_distribution_slices(image_path, label_path):
    # Load image
    ...
    # Sample 3 slices based on a normal distribution around the middle slice
    num_slices = img_data.shape[2]
    sampled_slices = np.clip(np.random.normal(loc=middle_index, scale=num_slices // 8, size=3).astype(int), 0, num_slices - 1)

    for i, slice_idx in enumerate(sampled_slices):
        # Extract the slice for image
        ...
        img_output_path = os.path.join(output_dir, f"{os.path.basename(image_path).split('.')[0]}_{i+1}.png")
        img.save(img_output_path)
        print(f"Saved {img_output_path}")

        # Save the label slice
        label_img = Image.fromarray(label_mask)
        label_output_path = os.path.join(output_dir, f"{os.path.basename(label_path).split('.')[0]}_mask_{i+1}.png")
        label_img.save(label_output_path)
        print(f"Saved {label_output_path}")

# Process all images and labels
for img_file, lbl_file in zip(image_files, label_files):
    save_normal_distribution_slices(img_file, lbl_file)
