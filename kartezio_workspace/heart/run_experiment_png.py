...
from numena.io.image import imread_color, imread_grayscale
from sklearn import mixture
from sklearn.decomposition import PCA
from sklearn.preprocessing import StandardScaler
from train_models_png import preprocessing

infos = ProfilingInfo("C:/Module9/Project/module9-dsai/kartezio_workspace/heart/dataset_png/META.json")
profiling = CellStainingProfile(infos)



def _read_image(filepath):
    img = imread_grayscale(filepath)
    channels = [
        img[:, 0],
        img[:, 1],
        img[:, 2],
...
def read_batch(directory: Directory):
    batch = {}
    for filepath in directory.ls("*.png", ordered=True):
...
if __name__ == "__main__":
    dataset = read_dataset("C:/Module9/Project/module9-dsai/kartezio_workspace/heart/dataset_png")
    # Load Kartezio Model
    model = KartezioModel(
        "C:/Module9/Project/module9-dsai/kartezio_workspace/heart/models/MCW/RGB/112971-3ccc142a-f2c5-4b8e-8dc4-b5ece9078643/elite.json", FitnessAP()
    )
    ...
    model_introspection = KartezioModel(
        "C:/Module9/Project/module9-dsai/kartezio_workspace/heart/models/MCW/RGB/746081-bfe548e3-10e9-4b71-a013-55c4f004217e/elite.json", FitnessAP()
    )
    ...
    # Load raw images
    batch_directory = Directory("C:/Module9/Project/module9-dsai/kartezio_workspace/heart/dataset_png")
    ...
    for image_name, p in pred_batch.items():
        if "NP" in image_name:
            ...
            condition = "P"
        labels = p["labels"]
        visual_image = imread_grayscale("C:/Module9/Project/module9-dsai/kartezio_workspace/heart/dataset_png/" + image_name)
        ...
        plot_watershed(
            ...
            p["markers"],
            p["labels"],
            filename=f"kartezio_{image_name}",
        )
    ...
    df = pd.DataFrame(cell_features, columns=cell_features)
    df["area"] = [c.area for c in cells_for_features]
    print(df.head())
    print("dataframe is : ",df.values.shape)
    ...
    with plt.style.context(["science", "nature"]):
        ...
        ax.legend()
        ax.set(**pparam)
        fig.savefig("C:/Module9/Project/module9-dsai/kartezio_workspace/heart/results/CTL_Target.png", dpi=300)
    ...
    for image_name in list(batch.keys()):
        filename = f"C:/Module9/Project/module9-dsai/kartezio_workspace/heart/results/kartezio_classes_{image_name}"
