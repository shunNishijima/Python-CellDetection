...
output_dir = "C:\\Module9\\Project\\module9-dsai\\kartezio_workspace\\heart\\dataset_png"
...
def save_middle_slice(image_path, label_path):
    ...
    img_output_path = os.path.join(output_dir, f"{os.path.basename(image_path).split('.')[0]}_.png")
    img.save(img_output_path)
    print(f"Saved {img_output_path}")

    # Save the label slice
    label_img = Image.fromarray(label_mask)
    label_output_path = os.path.join(output_dir, f"{os.path.basename(label_path).split('.')[0]}_mask.png")
